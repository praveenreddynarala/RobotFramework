*** Settings ***
Documentation     A Test Suite With Set Of Tests To Validate IXContract Create Complex Contract for SMS With Revert to Hubbing
...
Validate Set Configure Number Plan Call Type
    [Tags]    REGRESSION
    Verify Set Configure Number Plan Call Type      ${SMS CALL TYPE}
...
Validate Move Available countries To Destination
    [Tags]    REGRESSION
    Verify Move Available Countries To Destination      ${CONFIGURE NUMBER PLAN SMS COUNTRY}
...
Validate Move Available Countries To Destination For Aggregation Rules
    [Tags]    REGRESSION
    Verify Move Available Countries To Destination Deselecting First Item      ${CONFIGURE NUMBER PLAN SMS COUNTRY}
...
Validate Click Rating Structure 22 Arrow
    [Tags]    REGRESSION
    Verify Click Rating Structure 22 Arrow

Validate Set Rate Structure
    [Tags]    REGRESSION
    Verify Set Rate Structure       ${RATE STRUCTURE}

Validate Set Splitting Type
    [Tags]    REGRESSION
    Verify Set Splitting Type       ${SPLITTING TYPE}

Validate Set Threshold
    [Tags]    REGRESSION
    Verify Set Threshold

Validate Set Penalty Rate
    [Tags]    REGRESSION
    Verify Set Penalty Rate

Validate Click Rating Structure Save Button
    [Tags]    REGRESSION
