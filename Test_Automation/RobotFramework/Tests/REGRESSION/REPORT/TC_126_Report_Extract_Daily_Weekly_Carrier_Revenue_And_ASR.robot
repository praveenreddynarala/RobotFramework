*** Settings ***
Documentation     [TAENOSIS-816] A Test Suite With Set Of Tests For Verify Extract Daily/Weekly Carrier Revenue and ASR
...
Validate Click Specific Extract name From List
    [Tags]    REGRESSION
    Verify Click Specific Extract name From List        Extract - ${DAILY WEEKLY CARRIER REVENUE AND ASR SUBMENU}
...
Validate Set Report Group
    [Tags]    REGRESSION
    Verify Set Report Group     ${OPERATIONAL SALES REPORTS MENU}

Validate Set Report
    [Tags]    REGRESSION
    Verify Set Report       ${DAILY WEEKLY CARRIER REVENUE AND ASR SUBMENU} Report
