*** Settings ***
Documentation     [TAENOSIS-810] A Test Suite With Set Of Tests For Verify Extract Cost Revenue And ASR Report DA
...
Validate Click Extract - Suspect Rate Report Submenu
    [Tags]    REGRESSION
    Verify Click Specific Extract name From List        Extract - ${COST REVENUE AND ASR REPORT SUBMENU} (DA)

Validate Click Submit Button With ID
    [Tags]    REGRESSION
    Verify Click Extract Submit Button

Validate Cost Revenue and ASR MA Report Is Displayed
    [Tags]    REGRESSION
    Verify Specific Report Is Displayed        Cost, Revenue and ASR

Validate Close Detail Report Window
    [Tags]    REGRESSION
    Verify Close IxReport Window

Validate Set Extract Begin Month
    [Tags]    REGRESSION
    Verify Set Extract Begin Date           1/1/2015

Validate Click Submit Button With ID After Setting Begin Date
    [Tags]    REGRESSION
    Verify Click Extract Submit Button

Validate Close Detail Report Window After Setting Begin Date
    [Tags]    REGRESSION
    Verify Close IxReport Window
...
Validate Set Report Group
    [Tags]    REGRESSION
    Verify Set Report Group     ${DAILY ACCOUNTING REPORTS MENU}
...
Validate Set Extract Active Date
    [Tags]    REGRESSION
    Verify Set Extract Active Date      Current Month
