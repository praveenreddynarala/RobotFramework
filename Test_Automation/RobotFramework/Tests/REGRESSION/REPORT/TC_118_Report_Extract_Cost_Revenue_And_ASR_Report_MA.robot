*** Settings ***
Documentation     [TAENOSIS-811] A Test Suite With Set Of Tests For Verify Extract Cost Revenue And ASR Report MA
...
Validate Click Extract - Suspect Rate Report Submenu
    [Tags]    REGRESSION
    Verify Click Specific Extract name From List        Extract - ${COST REVENUE AND ASR REPORT SUBMENU} (MA)
...
Validate Click Submit Button With ID After Setting Begin Month
    [Tags]    REGRESSION
    Verify Click Extract Submit Button

Validate Close Detail Report Window After Setting Begin Month
    [Tags]    REGRESSION
    Verify Close IxReport Window
...
Validate Set Report Group
    [Tags]    REGRESSION
    Verify Set Report Group     ${MONTHLY ACCOUNTING REPORTS MENU}
