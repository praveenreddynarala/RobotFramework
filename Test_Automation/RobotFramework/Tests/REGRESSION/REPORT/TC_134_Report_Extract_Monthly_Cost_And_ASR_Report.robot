*** Settings ***
Documentation     [TAENOSIS-] A Test Suite With Set Of Tests For Verify Extract Monthly Cost and ASR Report
...
Validate Click Extract - Suspect Rate Report Submenu
    [Tags]    REGRESSION
    Verify Click Specific Extract name From List        Extract - ${MONTHLY COST AND ASR SUBMENU}
...
Validate The Specific Report Page Is Loaded Poperly
    [Tags]    REGRESSION
    Verify The Specific Report Page Is Loaded Poperly       ${MONTHLY COST AND ASR RREPORT LIST ITEM}
...
Validate Set Report
    [Tags]    REGRESSION
    Verify Set Report       ${MONTHLY COST AND ASR RREPORT LIST ITEM}
