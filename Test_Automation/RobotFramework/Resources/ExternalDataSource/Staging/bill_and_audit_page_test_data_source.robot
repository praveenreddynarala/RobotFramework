...
${CHARGE NAME PREFIX FOR ADD CHARGE}							UAT_AUTO_ChargeName_         # Charge name prefix, editable.
...
@{ADJUSTMENT FOLDER NAME}				            UAT_AUTO_PraveenTest							# Adjustment folder name list, editable.
...
@{ADJUSTMENT FOLDER NAME FOR ADD DISPUTE}				UAT_AUTO_PraveenTest							# Adjustment folder name list, editable.
${ADJUSTMENT FOLDER NAME FOR SELECT DISPUTE INLINE}		UAT_AUTO_PraveenTest							# Adjustment folder name, editable.
...
${NON RECURRING CHARGE PREFIX}          UAT_AUTO_Non Recurring Charge_      # Non recurring charge prefix, not editable.
...
${BULK ADD PRODUCT NAME}				        Angola-PSTN					# Product name, editable.
...
${NEW DOCUMENT NAME PREFIX}				        UAT_AUTO_Document_					# Document name prefix, editable.
...
${FILE PATH FOR UPLOAD}				            D:/OriginSetTemplate.xls			# Document file path, editable.
...
${NON RECURRING PRODUCT PREFIX}				                Product_					# Product prefix, editable.
...
${TRANSACTION CODE PREFIX}                          UAT_AUTO_Transaction Code           # Transaction code prefix, editable.
...
@{MAP LINE ITEM AUDIT ACCOUNT LIST}         SK_Tata_In      # Audit account list, editable.
@{MAP LINE ITEM STATEMENT STATUS LIST}      Entered         # Statement status list, not editable.
${MAP LINE ITEM STATEMENT NUMBER}           SK_RCNRC        # Statement number, editable.
...
${FILE PATH FOR UPLOAD STATEMENT}                    C:/InboundStatementUploadTemplate.xlsx                    # Statement upload file path, editable.
...
${DISPUTE NOTE PREFIX}						Test Automation_				# Note prefix, editable.
...
${DESCRIPTION PREFIX}						Test Automation_				# Description prefix, editable.
...
@{STATUS LIST FOR LOSE DISPUTE}				Disputed					    # Status list, editable.
${BEGIN DATE FOR LOSING DISPUTE}			11/23/2012					    # Begin date, editable.
${END DATE FOR LOSING DISPUTE}			    2/5/2017					    # End date, editable.
${LOSE DISPUTE DESCRIPTION PREFIX}			Test Automation_				# Description prefix, editable.
...
@{EXPORT STATEMENT STATUS LIST}         Mapped          # Statement status list, not editable.
${EXPORT STATEMENT STATUS}              Mapped          # Statement status, not editable.
...
${DISPUTE NEW DOCUMENT NAME PREFIX}				        UAT_AUTO_Document_				# Document name prefix, editable.
...
${DISPUTE FILE PATH FOR UPLOAD}				            C:/OriginSetTemplate.xls		# Document file path, editable.
...
${ADD RESOLUTION DESCRIPTION}				            Test Automation					# Description prefix, editable.
...
${DESCRIPTION FOR EDIT}				            UAT AUTO Test Automation				# Description prefix for edit, editable.
...
${CHARGE PREFIX FOR NON RECURRING CHARGE}                   UAT_AUTO_Test_                      # Non recurring charge prefix, editable.
...
${PRODUCT NAME FOR CREATE ADJUSTMENT}       Afghanistan-PSTN				# Create adjustment charge product name, editable.
...
${OUTBOUND DOCUMENT NAME PREFIX}				        UAT_AUTO_Document_					# Document name prefix, editable.
...
${OUTBOUND FILE PATH FOR UPLOAD}				            C:/OriginSetTemplate.xls			# Document file path, editable.
...
${TRANSACTION CODE PREFIX FOR ADD TRANSACTION}      UAT_AUTO_Transaction                # Transaction code prefix, editable.
...
${NON RECURRING CHARGE NAME PREFIX}				UAT_AUTO_Testing 		# Non recurring charge prefix, editable.
...
${PRODUCT NAME FOR ISSUING ADJUSTMENT}              Afghanistan-PSTN				# Adjustment charge product name, editable.
...
${FOLDER NAME PREFIX FOR ISSUING ADJUSTMENT}        UAT AUTO Test_				    # Adjustment folder name prefix, editable.
