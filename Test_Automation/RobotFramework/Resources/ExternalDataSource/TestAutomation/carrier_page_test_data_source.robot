...
${CREATE CARRIER POPUP NAME}                    Create Carrier              # Create Carrier PopUp Name, not Editable
...
${INTER OPCO NEW EVENT CASE POP UP NAME}		New Event Case - Event OpCo Scenario Group		# Inter Opco New event case pop up name, not Editable
${EDIT NUMBER PLAN POP UP NAME}		            Edit Number Plan for		# Edit Number Plan for pop up name, not Editable
...
${INBOUND TRAFFIC DIRECTION}						Inbound				    # Inbound Traffic direction item, Editable
${REFERENCE DESTINATION PRODUCT CATALOG}							    Reference Destination Rating Product 		# Reference Destination Product catalog item, Editable
...
${RESIDUAL TRAFFIC WINDOW NUMBER}					55							# Residual traffic window number, Editable.
${COVER LETTER TEMPLATE}							Declaration Template - USD		# Cover letter template, Editable.
${STATEMENT TEMPLATE}								Default Template             # Statement template, Editable.
...
${USAGE TIME ZONE FOR OUT AUDIT}							Settlement Call Date		# Usage time zone, Editable.
...
# Start: Test Data For Verify The Directions Are Displayed Correctly In Number Plan Creation
${INBOUND DIRECTION}                   Inbound               # Inbound direction, not editable.
# End: Test Data For Verify The Directions Are Displayed Correctly In Number Plan Creation